...
describe('The LESS instruction', () => {
    ...
    it('returns that NaN is not less than anything', () => {
        expect(getResult('NAN NAN')).toBe(0);
        expect(getResult('NAN INFINITY')).toBe(0);
        expect(getResult('INFINITY NAN')).toBe(0);
        expect(getResult('NAN 3')).toBe(0);
        expect(getResult('3 NAN')).toBe(0);
    });

    it('returns that INFINITY is not less than INFINITY', () => {
        expect(getResult('INFINITY INFINITY')).toBe(0);
    });

    it('returns that -INFINITY is less than INFINITY', () => {
        expect(getResult('-INFINITY INFINITY')).toBe(1);
    });
